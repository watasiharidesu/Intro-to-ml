...
# create conditions to check ineffective variables
nearZ = checkConditionalX(tr_n, factor(tr_n$target))
col_ct = sapply(tr_c, function(x) length(unique(x)))

# creat the target and related values for graphing purposes
y = tr$target
...
# exclude variables with over 50% are missing values
tr_m = tr[1,]
for (i in 1:ncol(tr)){
...
# seperate data based on data types
...
# summarize unique value information for each data type 
...
# exclude variables with only 2 unique values
